...
const Diaries = async ({ searchParams }: { searchParams: Promise<{ page?: string }> }) => {
  const { page = "1" } = await searchParams;
  const token = (await cookies()).get("token")?.value;
  const res = await fetch(`${process.env.LARAVEL_API_URL}/api/diaries?page=${page}`, {
    headers: {
  ...
  const diaries: Diary[] = fetchData.diaries.data;
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 motion-safe:animate-fade-up">
        {diaries.map((diary) => (
          ...
          >
            <img
              src={diary.cover_image ? `${process.env.LARAVEL_API_URL}/storage/${diary.cover_image?.path}` : `/placeholder.png`}
              alt="cover image"
