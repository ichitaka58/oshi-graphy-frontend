...
// ブラウザからのログインリクエストを受け取り、Laravelに転送する Route Handler。
// access_token をブラウザのJSに渡さず httpOnly Cookie に隠すことで XSS によるトークン盗難を防ぐ。
export async function POST (request: Request) {
  const body = await request.json();

  // res = Laravel からの HTTP レスポンス全体（status, headers, body を含む）
